...
export class Player {
    ...
    public __FASS_SIZE: u32 = 52 + this!.name.length;
    @inline __FASS_SERIALIZE(output: ArrayBuffer, input: Player): void {
        ...
        store<u64>(changetype<usize>(output), input.id, 3 + this!.name.length);
        ...
        store<u32>(changetype<usize>(output), load<u32>(changetype<usize>(input.data)), 48 + this!.name.length);
    }
    @inline __FASS_DESERIALIZE(input: ArrayBuffer, output: Player): void {
        ...
        output.id = load<u64>(changetype<usize>(input), 3 + this!.name.length);
        ...
        store<u32>(changetype<usize>(output.data), load<u32>(changetype<usize>(input), 48 + this!.name.length));
    }
}