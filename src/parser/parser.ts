...
export class Parser {
  ...
  parseSource(source: Source) {
    ...
    while (true) {
      ...
      if (currentToken == "include") {
        ...
        source.stmts.push(
          new CommentStatement(currentToken.slice(2), "single")
        );
  ...
  parseIncludeDeclaration(source: Source): IncludeDeclaration {
    ...
    if (isStringLiteral(currentToken!)) {
      ...
      if (!newSource)
        throw new Error(
          "Could not find source that was specified in the includes declaration"
        );
      ...
      for (const stmt of newSource.stmts) {
        if (
          ...
          source.scope.addElement(new ScopeElement(stmt.name.value, stmt));
      }
      return includeDecl;
    }

    throw new Error(
      "Expected path to included file, but found " + currentToken + " instead."
    );
  ...
  parseStructDeclaration(source: Source): StructDeclaration {
    ...
    // Add to scope
    source.scope.addElement(
      new ScopeElement(structDecl.name.value, structDecl)
    );
    return structDecl;
  }
